...
public class PresentAttributeQuery extends AttributeQuery {

    public static final String KEY = "DEVICE_PRESENT";

    public PresentAttributeQuery(final Condition.Operator operator, final Object value) {
        super(operator, value);
    }

    public PresentAttributeQuery(final RuleConditionV1 ruleConditionV1) {
    ...
    public RuleConditionV1 getCondition() {
        return new RuleConditionV1().withName(getKey()).withAllOf(
                Arrays.asList(
                        ...
                        new RuleConditionV1().withCondition(
                                new Condition().withObject("event.value").withOperator(EQ).withValue(Hardware.Presence.PRESENT.toString())
                        )
                )
        );
    }
